#!/usr/bin/env python3
"""
HL2 Elysium Dream Launcher
============================
Boots the Twilight Elysium dream engine with Half-Life 2-themed data and scripts.
...
class HL2DreamEngine(ContentDreamEngine):
    """Dream engine preset for the Half-Life 2 theme."""
    ...
    def __init__(self, difficulty: str = "casual"):
        ...
        if not self.dream_generator.script_state.weapons and not self.dream_generator.script_state.enemies:
            print("[HL2] No script data loaded; Content/Data JSONs are optional.")
...
def main() -> int:
    print("[HL2] Starting Half-Life 2 Elysium Dream Engine...")
    difficulty = "casual"
    if len(sys.argv) > 1 and sys.argv[1] in ("casual", "legend"):
        difficulty = sys.argv[1]
    engine = HL2DreamEngine(difficulty=difficulty)
    diff = engine.difficulty
    print(f"[HL2] Difficulty: {diff['profile']}")
    if diff["settings"]:
        print(f"[HL2]   player  -> {diff['settings'].get('player')}")
        print(f"[HL2]   combat  -> {diff['settings'].get('combat')}")
    scene = engine.dream()
    print(f"[HL2] Generated scene with {len(scene.objects)} objects")
    return 0
