...
bool Game::initialize() {
    m_weapons.load("Content/Data/HL2_Weapons.json");
    m_enemies.load("Content/Data/HL2_Enemies.json");
    m_maps.load("Content/Data/HL2_Maps.json");
    m_diff.load("Content/Data/Difficulties.json");
    m_npcs.load("data/npcs.json");

    if (m_npcs.npcs().empty()) {
        register_hl2_npcs(m_npcs);
    }
...
void Game::show_menu() {
    std::printf("\x1b[38;5;202m\x1b[1m");
    std::printf("  HALF-LIFE 2 ELYSIUM: CITY 17\n");
    std::printf("  ============================\n");
    std::printf("\x1b[0m");
    std::printf("  \x1b[2mWelcome to City 17. Welcome to your destiny.\x1b[0m\n\n");
...
bool Game::run_chapter(uint32_t idx) {
    ...
    std::printf("\n  \x1b[38;5;202m\x1b[1mCHAPTER %u: %s\x1b[0m\n", desc->chapter, desc->name.c_str());

    std::string map_path = "Content/Maps/" + std::string(desc->name == "City 17" ? "city17" :
        desc->name == "Ravenholm" ? "ravenholm" :
        desc->name == "Nova Prospekt" ? "nova_prospekt" :
        desc->name == "C17 Tower (Citech)" ? "c17_tower" : "city17") + ".json";
    if (!m_world.load_from_json(map_path)) {
        m_world.generate_from_desc(*desc, idx * 7919 + 1);
    }

    // Place NPCs
    auto npc_list = m_npcs.in_world("city17");
...
void Game::world_loop() {
    std::printf("  \x1b[2mwasd=move | j=attack | h=heal | r=reload | t=talk | g=gravity gun | q=quit\x1b[0m\n");
    draw_world();
...
void Game::draw_world() {
    ...
    std::printf("  \x1b[2mHP: %.0f/%0.0f | Armor: %u%% | Ammo: %u | Gravity Gun: %s\x1b[0m\n",
                m_state.player.hp, m_state.player.max_hp, m_state.player.suit,
                m_state.player.ammo, m_state.player.gravity_gun_unlocked ? "YES" : "NO");
}

void Game::handle_command(const std::string& cmd) {
    if (cmd == "?" || cmd == "help") {
        std::printf("  wasd=move j=attack h=heal r=reload t=talk g=gravity gun\n");
        return;
    }
    ...
    for (char c : cmd) {
        if (c == 'w') ny--;
            ...
            auto e = m_enemies.of_type("soldier");
            if (e.empty()) e = m_enemies.of_type("beast");
            if (e.empty()) e = m_enemies.of_type("robotic");
            if (e.empty() && !m_enemies.enemies().empty())
                start_combat(m_enemies.enemies()[std::rand() % m_enemies.enemies().size()]);
            else if (!e.empty())
                start_combat(*e[std::rand() % e.size()]);
        }
        else if (c == 'h') {
            m_state.player.hp = std::min(m_state.player.max_hp, m_state.player.hp + 25);
            std::printf("  \x1b[38;5;46m+25 HP\x1b[0m\n");
        }
        else if (c == 'r') std::printf("  \x1b[38;5;220mReloaded.\x1b[0m\n");
        else if (c == 'g') {
            if (m_state.player.gravity_gun_unlocked) {
                std::printf("  \x1b[38;5;141mGravity Gun: %s\x1b[0m\n",
                            m_state.player.suit >= 50 ? "POWER 2 (launch objects)" : "POWER 1 (fetch)");
            } else {
                std::printf("  \x1b[2mGravity Gun not yet acquired.\x1b[0m\n");
            }
        }
        else if (c == 't') {
            auto npc_list = m_npcs.in_world("city17");
            if (!npc_list.empty()) encounter_npc(*npc_list[std::rand() % npc_list.size()]);
        }
...
void Game::encounter_npc(const NPCDef& npc) {
    std::printf("\n  \x1b[38;5;81m\x1b[1m[%s]\x1b[0m \x1b[2m%s\x1b[0m\n",
                npc.name.c_str(), npc.affiliation.c_str());
...
void Game::start_combat(const EnemyDef& enemy) {
    std::printf("\n  \x1b[38;5;196mA %s blocks your path!\x1b[0m\n", enemy.name.c_str());
    ...
    if (!wep) { std::printf("No weapon.\n"); return; }

    CombatEngine combat(m_enemies, m_diff);
    combat.engage(enemy, m_state.player.hp, m_state.player.max_hp, *wep);

    std::printf("  \x1b[1mCOMBAT: %s | HP: %u/%u\x1b[0m\n",
                enemy.name.c_str(), combat.enemy_hp(), combat.enemy_max_hp());
    ...
    if (combat.player_won()) {
        ...
        std::printf("  \x1b[38;5;46mVictory! The %s goes down.\x1b[0m\n", enemy.name.c_str());
    } else if (combat.state() == CombatState::defeat) {
        m_state.game_over = true;
        std::printf("  \x1b[38;5;196mGordon Freeman. Added to the list.\x1b[0m\n");
    }
